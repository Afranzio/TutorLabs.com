...
const validateForgetPassword = reqBody => {
  Joi.validate(reqBody, userValidatorSchema.validateForgetPassword, error => {
    if (error) {
      throw new Error(error.details[0].message);
    }
  });
...
const validateCreateUser = reqBody => {
  Joi.validate(reqBody, userValidatorSchema.validateCreateUser, error => {
    if (error) {
      throw new Error(error.details[0].message);
    }
  });
